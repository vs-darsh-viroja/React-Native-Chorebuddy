...
type HouseholdValue = {
  ...
  addMember(name: string, avatar: string, photoData?: string): Promise<void>; updateMember(id: string, name: string, avatar: string, photoData?: string): Promise<void>; deleteMember(id: string): Promise<void>; makeAdmin(id: string): Promise<void>; leaveHousehold(): Promise<void>;
};
...
export function HouseholdProvider({ children }: React.PropsWithChildren) {
  ...
  const attach = useCallback((householdId: string) => {
    ...
    unsubscribers.current = [
      ref.onSnapshot(snapshot => { const parsed = parseHousehold(snapshot); if (parsed) { setHousehold(parsed); setLoadState('ready'); setWorking(false); } else if (!snapshot.metadata.fromCache) { setHousehold(null); setMembers([]); setLoadState('noHousehold'); } }, next => { setError(next.message); setLoadState('noHousehold'); }),
      ref.collection('members').orderBy('createdAt').onSnapshot(snapshot => setMembers(snapshot.docs.map(parseMember).filter((value): value is HouseholdMember => Boolean(value))), next => setError(next.message)),
    ];
  }, [detach]);
  useEffect(() => {
  ...
  const createHousehold = async (name: string, adminName: string, adminAvatar: string, memberNames: string[]) => {
    ...
    try { const code = await uniqueCode(); const ref = db.collection('households').doc(); await ref.set({ name, inviteCode: code, adminUserId: user.uid, adminUserIds: [user.uid], createdAt: firestore.FieldValue.serverTimestamp() }); const admin = ref.collection('members').doc(); await admin.set({ name: adminName, avatar: adminAvatar, isAdmin: true, claimedByUserId: user.uid, createdAt: firestore.FieldValue.serverTimestamp() }); for (let index = 0; index < memberNames.length; index += 1) await ref.collection('members').doc().set({ name: memberNames[index], avatar: avatars[(index + 1) % avatars.length], isAdmin: false, claimedByUserId: null, createdAt: firestore.FieldValue.serverTimestamp() }); await db.collection('users').doc(user.uid).set({ currentHouseholdId: ref.id, memberId: admin.id, displayName: adminName }, { merge: true }); setMyMemberId(admin.id); attach(ref.id); } catch (next) { setError(next instanceof Error ? next.message : String(next)); setWorking(false); }
  };
  ...
  const finishClaim = async (householdId: string, memberId: string, displayName: string) => { if (!user) return; await db.collection('users').doc(user.uid).set({ currentHouseholdId: householdId, memberId, displayName }, { merge: true }); setMyMemberId(memberId); attach(householdId); };
  ...
  const deleteMember = async (id: string) => { if (!household) return; const target = members.find(item => item.id === id); if (!target || target.isAdmin || target.claimedByUserId) throw new Error('Claimed or admin profiles cannot be deleted.'); await memberRef(id).delete(); };
  ...
  const leaveHousehold = async () => { if (!household || !user || !myMemberId) return; const me = members.find(item => item.id === myMemberId); const others = members.filter(item => item.id !== myMemberId && item.claimedByUserId); const houseRef = db.collection('households').doc(household.id); if (others.length) { const batch = db.batch(); batch.update(memberRef(myMemberId), { claimedByUserId: null, isAdmin: false }); if (me?.isAdmin) { const heir = others.find(item => item.isAdmin) ?? others[0]; batch.update(memberRef(heir.id), { isAdmin: true }); batch.update(houseRef, { adminUserId: heir.claimedByUserId, adminUserIds: firestore.FieldValue.arrayRemove(user.uid) }); } batch.set(db.collection('users').doc(user.uid), { currentHouseholdId: '', memberId: '' }, { merge: true }); await batch.commit(); } else { await db.collection('users').doc(user.uid).set({ currentHouseholdId: '', memberId: '' }, { merge: true }); } detach(); setHousehold(null); setMembers([]); setMyMemberId(null); setLoadState('noHousehold'); };
  const value = useMemo<HouseholdValue>(() => ({ loadState, household, members, myMemberId, isWorking, error, clearError: () => setError(null), createHousehold, lookupHousehold, claimMember, createAndClaimMember, addMember, updateMember, deleteMember, makeAdmin, leaveHousehold }), [loadState, household, members, myMemberId, isWorking, error, attach, user]);
